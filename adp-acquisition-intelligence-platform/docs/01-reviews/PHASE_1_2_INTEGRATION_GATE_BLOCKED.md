# Phase 1.2 Integration Gate — Status

**Date:** 2026-07-24  
**Canonical PR:** [#22](https://github.com/mikemiller1425-design/adp-acquisition-intelligence-platform/pull/22)  
**Branch:** `cursor/phase-1-2-live-research-orchestration-fb9d`  
**Supersedes:** [#21](https://github.com/mikemiller1425-design/adp-acquisition-intelligence-platform/pull/21) (`cursor/phase-1-2-live-research-orchestration-dd2b`, branch retained)

## Entry gate (section 1) — cleared

The repair brief required:

> PR #20 must merge first. … **Stop if PR #20 is not merged.**

### Verified at merge / rebase

| Item | Result |
|---|---|
| PR #20 state | **MERGED** |
| PR #20 head (pre-merge) | `d9a810897fe15c126af83b32fae83197ae3acc5b` |
| Merge strategy | Local `--no-ff` merge commit pushed to `main` (`gh pr merge` unavailable / read-only) |
| Resulting `main` SHA | `685b1a543c618e941e2447947728d9f075b585bc` |
| PR #22 rebase onto post-#20 `main` | **Done** — Phase 1.1 files absent from effective diff |
| PR #21 | **Superseded** by #22; branch not deleted; do not merge |

## Original blocked findings (from PR #21)

When #20 was still open, integration-gate repair correctly stopped. Deferred work:

1. Rebase onto updated `main`; remove inherited Phase 1.1 noise from the effective diff
2. Connect Next.js `postgres` + `ADP_JOB_QUEUE=durable` launches to `DurableJobDispatcher` / `PostgresDurableJobStore` (fail closed; no silent memory fallback)
3. Canonical `SourceRegistryPort` from `approved_sources` (remove synthetic `fixtureSourcesFor()` for PostgreSQL)
4. Explicit adapter composition; no unconfigured hidden defaults
5. No fake PostgreSQL target org IDs; resolve saved target segments only
6. Worker lease / heartbeat / recovery / checkpoint idempotency
7. Separate PostgreSQL durable Playwright E2E (Next + worker + PG through 0012)
8. Zero-egress probe suite
9. Docs: claim persistent queue/worker only after PG worker E2E → **READY FOR PERSISTENT FIXTURE PILOT**

## Repair status on PR #22

| Item | Status |
|---|---|
| Web → Postgres `durable_jobs` (`processInline: false`) | Implemented |
| Separate worker execution + lease reclaim | Implemented |
| Canonical `SourceRegistryPort` / `approved_sources` | Implemented |
| Persisted target-segment organizations (real UUIDs) | Implemented |
| PostgreSQL Playwright orchestration | **PASS** (`test:e2e:research-run-postgres`) |
| Web / worker restart recovery | **PASS** (durable E2E) |
| Zero-egress unit probes | **PASS** (`research-run.unit.test.ts`) |

## Explicit non-goals (unchanged)

- Do **not** merge PR #22 without separate approval
- Do **not** close RB-001–RB-017
- Do **not** enable live egress by default
- Do **not** claim READY FOR CONTROLLED EXTERNAL PILOT while RB-014–017 remain open
- Do **not** claim the durable queue/worker boundary works until the PostgreSQL worker E2E passes (see test report evidence)

## Historical note

PR #21 recorded this gate as **BLOCKED** while #20 was open. That finding remains accurate for that point in time; this document on #22 records the cleared gate and the repair carried forward.
