...
Prompt 1 must record whether Phase 1 is single-tenant or multi-tenant. If multi-tenant, every business table includes `tenant_id`, composite uniqueness includes it, row-level access is tested, and background jobs carry tenant scope. This decision cannot be postponed beyond the first schema migration. Consent and suppression tables follow the same tenant rule.
