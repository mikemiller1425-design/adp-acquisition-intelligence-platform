...
|---|---|---|---|
| UI-01 | `/dashboard` | Executive overview | filter period/owner/territory, drill into metrics |
| UI-02 | `/prospects` | Prospect master table | filter, save view, bulk assign/tag, export |
| UI-03 | `/prospects/new` | Manual organization/contact entry | validate, duplicate check, create |
| UI-04 | `/prospects/:id` | Organization 360 | inspect summary, variables, evidence, scores, activity, next action |
| UI-05 | `/imports` | Import history and start | upload, map, dry run, inspect errors, commit/revert |
...
| UI-08 | `/research` | Research queue/table | prioritize gaps, assign, open evidence entry |
...
| UI-18 | `/outreach` | Outreach operations table | select sequence, review drafts, record activity/response |
| UI-19 | `/outreach/templates` | Template/sequence library | draft, approve, version, retire |
| UI-20 | `/opportunities` | Opportunity/stage table | advance stage, flag risk, record outcome |
...
| UI-26 | `/audit` | Audit search | inspect actor/action/subject; restricted export |

## Organization 360 layout

Header: identity, aliases, firm type, owner, territory, prospect state, primary/secondary motion, completeness/confidence, last activity, next action. Tabs: overview, variables, evidence, scores, qualification, discovery, outreach, opportunities, tasks/notes, history. Score cards always display state/version, confidence, completeness, positive/negative factors, and high-impact gaps.
...
- Tables allow column choice, fixed identity column, keyboard navigation, accessible labels, pagination, and URL-addressable filters.
...
- User-facing dates show timezone; exports document UTC convention.

## Permissions

Researchers can propose evidence/values; reviewers accept conflicts/merges and qualification decisions; sales users manage discovery/outreach/opportunities within assignments; administrators publish definitions and manage access. Permissions are server-enforced and UI-hidden/disabled states are convenience only.

See [Dashboard Specification](../10-dashboards/DASHBOARD_SPECIFICATION.md) and [Functional Specification](../02-functional/PHASE_1_FUNCTIONAL_SPECIFICATION.md).
