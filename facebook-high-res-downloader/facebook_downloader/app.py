"""Main CustomTkinter window for Facebook High-Res Downloader."""
...
from facebook_downloader import __app_name__, __version__
from facebook_downloader.config import AppConfig
...
from facebook_downloader.validators import extract_facebook_urls
...
class FacebookDownloaderApp(RootWindow):  # type: ignore[valid-type, misc]
    """Professional, minimal desktop UI for high-res Facebook downloads."""

    PAD = 18
    ACCENT = "#1877F2"  # Facebook blue for the primary CTA
    ...
    def __init__(self) -> None:
        ...
        self.geometry(self.config_store.get("window_geometry") or "920x740")
    ...
    def _build_ui(self) -> None:
        ...
        root.grid_rowconfigure(5, weight=1)  # log grows

        self._build_header(root)
        self._build_url_section(root)
        self._build_destination_section(root)
        self._build_cookies_section(root)
    ...
    def _build_header(self, parent: ctk.CTkFrame) -> None:
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="Download original-quality Facebook photos & video via gallery-dl",
    ...
    def _build_url_section(self, parent: ctk.CTkFrame) -> None:
        ...
        self.url_box.insert(
            "1.0",
            "Paste one or more Facebook links here…\n"
        ...
        hint = ctk.CTkLabel(
            btn_row,
            text="Photos, videos, albums, posts, groups & profiles supported",
    ...
    def _build_cookies_section(self, parent: ctk.CTkFrame) -> None:
        """Facebook often needs logged-in cookies — expose a simple selector."""
        section = ctk.CTkFrame(parent, corner_radius=10)
        ...
        ctk.CTkLabel(
            section,
            text="Recommended for profiles, private content, and to avoid login walls",
            font=ctk.CTkFont(size=11),
            text_color=("gray45", "gray55"),
            anchor="w",
        ...
        self.cookies_menu = ctk.CTkOptionMenu(
            section,
            values=["None", "chrome", "chromium", "firefox", "edge", "brave", "opera"],
            width=160,
            command=self._on_cookies_change,
        )
        saved = self.config_store.cookies_from_browser
        allowed = {"chrome", "chromium", "firefox", "edge", "brave", "opera"}
        self.cookies_menu.set(saved if saved in allowed else "None")
        self.cookies_menu.grid(row=0, column=1, sticky="e", padx=14, pady=(12, 4))

    def _build_actions(self, parent: ctk.CTkFrame) -> None:
        row = ctk.CTkFrame(parent, fg_color="transparent")
        row.grid(row=4, column=0, sticky="ew", pady=(0, 12))
    ...
    def _build_progress_and_log(self, parent: ctk.CTkFrame) -> None:
        section = ctk.CTkFrame(parent, corner_radius=10)
        section.grid(row=5, column=0, sticky="nsew")
        section.grid_columnconfigure(0, weight=1)
    ...
    def _on_cookies_change(self, value: str) -> None:
        self.config_store.cookies_from_browser = "" if value == "None" else value
        self.config_store.save()
        if value == "None":
            self._set_status("Browser cookies disabled")
        else:
            self._set_status(f"Will use cookies from {value}")
    ...
    def _valid_urls(self) -> list[str]:
        return extract_facebook_urls(self._current_url_text())
    ...
    def _refresh_destination_ui(self) -> None:
        ...
        if path and Path(path).is_dir():
            self.dest_check.configure(text="✓", text_color="#2BB673")
            self.dest_title.configure(text="Folder selected")
            self.dest_path_label.configure(text=path)
            self.drop_zone.configure(border_color="#2BB673")
            self.open_folder_btn.configure(state="normal")
        else:
    ...
    def _check_gallery_dl(self) -> None:
        if gallery_dl_available():
            ...
            self._set_status(
                "Warning: gallery-dl not found — install with pip install gallery-dl"
            )
            self._append_log(
    ...
    def _cookies_choice(self) -> str | None:
        value = self.cookies_menu.get()
    ...
    def _start_download(self) -> None:
        urls = self._valid_urls()
        if not urls:
            messagebox.showwarning(__app_name__, "Add at least one valid Facebook URL.")
            return
        ...
        cookies = self._cookies_choice()
        if not cookies:
            self._append_log(
                "Note: no browser cookies selected. "
                "If downloads fail with login/rate-limit errors, choose Chrome or Firefox above."
            )
