"""gallery-dl subprocess wrapper for high-resolution Facebook downloads."""
...
def find_gallery_dl() -> str | None:
    """Locate a gallery-dl executable (PATH binary or ``python -m gallery_dl``)."""
...
def build_gallery_dl_command(
    ...
    *,
    ...
    - Prefers original / highest quality media (including videos)
    - Uses sensible filenames (username + media id when available)
    - Optionally loads browser cookies (often required by Facebook)
    - Runs modest concurrency to reduce rate-limit risk
    """
    ...
    # Filename: username + id when available (Facebook keywords).
    cmd.extend(
        [
            "-o",
            "filename={username|facebook}_{id}.{extension}",
        ]
    )

    # Include videos at best available quality.
...
class DownloadEngine:
    """Runs gallery-dl in a background thread and streams output to the UI."""
                    ...
                    if code == 0 or result.successful > 0:
                        ...
                        if on_log and result.successful == 0 and result.failed == 0:
                            on_log(
                                ...
                                "Facebook often requires browser cookies — "
                                "try selecting Chrome/Firefox in Settings, "
                                "or check for private / login-walled content."
                            )
