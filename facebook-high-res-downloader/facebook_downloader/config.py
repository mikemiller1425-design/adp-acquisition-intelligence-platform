...
def _default_config_path() -> Path:
    ...
    home = Path.home()
    xdg = home / ".config" / "facebook-high-res-downloader"
    if xdg.parent.exists() or Path("/etc/xdg").exists():
        xdg.mkdir(parents=True, exist_ok=True)
        return xdg / "settings.json"
    folder = home / ".facebook-high-res-downloader"
...
class AppConfig:
    ...
    DEFAULTS: dict[str, Any] = {
        ...
        "window_geometry": "920x740",
        "cookies_from_browser": "",  # e.g. "chrome", "firefox" — often required
    }
    ...
    def cookies_from_browser(self) -> str:
        return str(self._data.get("cookies_from_browser") or "").strip().lower()

    @cookies_from_browser.setter
    def cookies_from_browser(self, value: str) -> None:
        self._data["cookies_from_browser"] = (value or "").strip().lower()
