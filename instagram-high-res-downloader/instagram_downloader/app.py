"""Main CustomTkinter window for Instagram High-Res Downloader."""
...
from instagram_downloader import __app_name__, __version__
from instagram_downloader.config import AppConfig
...
from instagram_downloader.validators import extract_instagram_urls
...
class InstagramDownloaderApp(RootWindow):  # type: ignore[valid-type, misc]
    """Professional, minimal desktop UI for high-res Instagram downloads."""

    PAD = 18
    ACCENT = "#E1306C"  # Instagram-adjacent accent for the primary CTA
    ...
    def _build_header(self, parent: ctk.CTkFrame) -> None:
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="Download original-quality Instagram photos & video via gallery-dl",
    ...
    def _build_url_section(self, parent: ctk.CTkFrame) -> None:
        ...
        self.url_box.insert(
            "1.0",
            "Paste one or more Instagram links here…\n"
        ...
        hint = ctk.CTkLabel(
            btn_row,
            text="Posts, reels, IGTV, profiles & stories supported",
    ...
    def _build_cookies_section(self, parent: ctk.CTkFrame) -> None:
        """Instagram often needs logged-in cookies — expose a simple selector."""
        section = ctk.CTkFrame(parent, corner_radius=10)
    ...
    def _valid_urls(self) -> list[str]:
        return extract_instagram_urls(self._current_url_text())
    ...
    def _start_download(self) -> None:
        urls = self._valid_urls()
        if not urls:
            messagebox.showwarning(__app_name__, "Add at least one valid Instagram URL.")
            return
