"""gallery-dl subprocess wrapper for high-resolution VSCO downloads."""
...
def build_gallery_dl_command(
    ...
    *,
    ...
    - Uses sensible filenames (username + media id when available)
    ...
    # Filename: username + media id when available (falls back gracefully).
    cmd.extend(["-o", "filename={username|vsco}_{id}.{extension}"])
    ...
    def start(
        ...
        *,
        ...
        def _runner() -> None:
            ...
            try:
                cmd = build_gallery_dl_command(urls, dest, jobs=jobs)
                if on_log:
                    on_log(f"$ {' '.join(_shell_quote(c) for c in cmd[:8])} … ({len(urls)} URL(s))")
                ...
                self._proc = subprocess.Popen(
                    ...
                    # On Windows, CREATE_NEW_PROCESS_GROUP helps signal handling;
                    # elsewhere default is fine.
                )
                    ...
                    if code == 0 or result.successful > 0:
                        ...
                        if on_log and result.successful == 0 and result.failed == 0:
                            on_log(
                                ...
                                "This can happen with private content, "
        ...
        self._thread = threading.Thread(target=_runner, name="gallery-dl-runner", daemon=True)
        self._thread.start()
...
def _classify_line(line: str) -> tuple[bool, bool, bool]:
    ...
    """
    lower = line.lower()
    # Successful download lines typically start with "#" path or contain "# "
    # gallery-dl prints paths of written files prefixed with "# ".
